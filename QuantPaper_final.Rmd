...
```{r}
standard_freqwords$q + theme_minimal() #show only q plot
```

```{r results="hide"}
#my_pca <- JSTOR_clusterbywords(...) #reduce pca output
```
...
```{r}
commodity_freqwords$q + theme_minimal() #show only q plot
```
...
```{r}
pottery_freqwords$q + theme_minimal() #show only q plot
```
...
```{r}
ceramics_freqwords$q + theme_minimal() #show only q plot
```
