...
```{r Data_ingest}
library(JSTORr)
multiple_archives <- JSTORr ::: JSTOR_unpack_multiple_archives()
```

Codes from the JSTORr package will be utilized for the statistical analysis. Potential codes include the following:
```{r potential_codes}
JSTOR_2word(multiple_archives, "sago", "ceramics")
...
The results of these plots indicate that while there may not be a large number of instances in which these sets of words are appear together in the literature, each plot demonstrates that of the instances in which they do appear, there are a number of in which the sets of words are strongly correlated. This suggests, and close readings can verify this, that the data set for this research question would be rich enough to justify a quantitative method supplemented by a close readings of selected articles. 
